 ...
 *
 * usage:
 * Wind.css(); load css files
 * Wind.use(); load js files
 */
...
;(function(win) {
	var root = win.GV.WEB_ROOT+win.GV.JS_ROOT || location.origin + '/public/js/', //在wind.js加载之前定义GV.JS_ROOT
		...
		pro = function(alias,type) {//add suffix and version for JS and CSS!
				for(var i in alias) {
					if (alias.hasOwnProperty(i)) {
						alias[i] = root + alias[i] +'.'+type+'?v=' + ver;
					}
				}
		};
...
/*
	...
	*/
	...
	//Using the alias to load the file for javascript
	Wind.use = function() {
		var args = arguments,len = args.length;
        for( var i = 0;i < len;i++ ) {
        	if(typeof args[i] === 'string' && alias[args[i]]) {//过滤回调函数，并使用简写就可以加载相应的文件
        		args[i] = alias[args[i]];
        	}
        }
		Wind.load.apply(null,args);
	};
